...
def load_scheme(zoomed_image, prompt, image_name):
    try:
        ...
        # instantiate new enricher & retriever for this image
        enricher  = Enricher(device=f"cuda:{GEMMA_GPU}", st_device=f"cuda:{ST_GPU}")
        query_txt = enricher.build_query(zoomed_image, scheme["prompt"], image_name)
        retriever = Retriever(device=f"cuda:{ST_GPU}")
        scheme["guidelines"] = retriever.fetch(query_txt, k=1)
...
def main():
    ...
    # parser = argparse.ArgumentParser(description='Process an image for segmentation')
    # parser.add_argument('--image', type=str, default="2.jpeg",
    ...
    pattern = os.path.join(SRC_ROOT_IMG, "*.jpg")
    ...
    for i in range(len(image_folder)):
        ...
        try:
            ...
            out_dir = Path(OUT_ROOT, image_name)
            ...
            gt_json = json.load(open(os.path.join(SRC_ROOT_IMG, image_name + ".json")))
            text_gt = gt_json["text"][0]
            ...
            crops_to_process = [(crop, coords)]

            for crop_idx, (crop, coords) in enumerate(crops_to_process):
                if crop is not None:
                    zoomed_image = image  # use the zoomed crop for downstream processing
                else:
                    print(f"No crop generated for prompt '{prompt}'. Using full image.")
                    zoomed_image = image
                    coords = (0, 0, image.size[0], image.size[1])

                policy = load_scheme(zoomed_image, prompt, image_name)
                ...
                issue_cnt_prev  = _issue_count(sup_json)
                tiny_steps = 0
                TOTAL_ITERS_USED = 0
                ...
                for iter_idx in range(1, MAX_ITERS + 1):
                    ...
                    if candidate_present:
                        ...
                        # filter out keys with similarity score less than 0.5
                        ...
                        # with open(os.path.join(out_dir, "jsons", f"supervisor_critique_{crop_idx}.json"), "a") as f:
            ...
            # save the final mask as a 0-1 PNG (binary mask)
